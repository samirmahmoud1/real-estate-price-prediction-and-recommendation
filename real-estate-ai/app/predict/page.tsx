...
export default function PredictPage() {
  ...
  const [activeInsight, setActiveInsight] = useState(
    "Choose the property details, then run the prediction engine to compare against real properties from your dataset."
  );
  ...
  async function runPrediction() {
    setLoading(true);
    setActiveInsight("Searching nearest comparable properties...");
    ...
    try {
      ...
      if (data.error) {
        ...
        setResult(data);
        setActiveInsight(
          `Prediction completed using ${data.comparablesUsed} real comparable properties.`
        );
      }
    } catch {
      setActiveInsight("Backend is not running. Start FastAPI on port 8000.");
    }
      ...
      <section className="relative z-10 mx-auto max-w-7xl">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
          <div className="inline-flex items-center gap-2 rounded-full border border-cyan-300/20 bg-cyan-300/10 px-4 py-2 text-sm font-bold text-cyan-200">
            ...
            Real Comparable Pricing Engine
          </div>
          ...
          <p className="mt-5 max-w-3xl text-lg leading-8 text-slate-300">
            Clean AI valuation workspace connected to your real dataset. Select location, property profile,
            and confidence level, then compare the predicted value against nearest comparable properties.
          </p>
        </motion.div>

        <div className="mb-8 grid gap-5 md:grid-cols-4">
          <SummaryCard icon={<Gem />} title="Predicted Price" value={result ? formatCurrency(result.predictedPrice) : "--"} />
          <SummaryCard icon={<TrendingUp />} title="Median Comparable" value={result ? formatCurrency(result.medianComparablePrice) : "--"} />
          <SummaryCard icon={<ShieldCheck />} title="Comparable Count" value={result ? `${result.comparablesUsed}` : "--"} />
          <SummaryCard icon={<CheckCircle2 />} title="Market Delta" value={result ? formatCurrency(delta) : "--"} />
        </div>
        ...
        <div className="grid gap-8 xl:grid-cols-[0.92fr_1.08fr]">
          ...
          >
            <div className="mb-7 flex items-center justify-between">
              <div>
                <p className="text-sm text-slate-400">Prediction Control Panel</p>
            ...
            <div className="space-y-6">
              <div>
                <p className="mb-3 text-sm font-bold text-slate-300">Location</p>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <SelectBox label="Property Type" value={propertyType} options={options.propertyTypes} onChange={setPropertyType} />
                <SelectBox label="Tenure" value={tenure} options={options.tenures} onChange={setTenure} />
              </div>

              <div className="grid gap-4 md:grid-cols-3">
                <SmallNumber icon={<BedDouble size={17} />} label="Bedrooms" value={bedrooms} min={0} max={8} onChange={setBedrooms} />
                <SmallNumber icon={<Bath size={17} />} label="Bathrooms" value={bathrooms} min={0} max={8} onChange={setBathrooms} />
                <SmallNumber icon={<Home size={17} />} label="Living Rooms" value={livingRooms} min={0} max={5} onChange={setLivingRooms} />
              </div>

              <RangeControl label="Floor Area" value={area} min={25} max={320} step={5} suffix="sqm" onChange={setArea} />

              <div>
                <p className="mb-3 text-sm font-bold text-slate-300">Confidence Mode</p>
          ...
          <div className="space-y-6">
            <div className="rounded-[2rem] border border-cyan-300/20 bg-gradient-to-br from-cyan-300/10 via-white/[0.05] to-purple-500/10 p-7 shadow-2xl backdrop-blur-2xl">
              ...
              <p className="min-h-20 text-lg leading-8 text-slate-200">{activeInsight}</p>
            </div>
            ...
            <div className="grid gap-6 lg:grid-cols-[1fr_0.78fr]">
              <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-black/40 shadow-2xl">
                <div className="flex items-center justify-between border-b border-white/10 px-6 py-4">
                  <div>
                    <p className="text-sm text-slate-400">Google Maps Preview</p>
              ...
              <div className="space-y-6">
                <div className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-6 shadow-2xl backdrop-blur-xl">
                  ...
                  <p className="leading-7 text-slate-300">
                    {result ? (
                      <>
                        Status: <span className="font-black text-cyan-300">{result.marketStatus}</span>. Median comparable price is{" "}
                        <span className="font-black text-emerald-300">{formatCurrency(result.medianComparablePrice)}</span>.
                      </>
                    ) : (
                ...
                <div className="rounded-[2rem] border border-white/10 bg-[#07111f] p-6 shadow-2xl">
                  <p className="text-sm uppercase tracking-[0.25em] text-cyan-300">Current Profile</p>
                  <h3 className="mt-3 text-2xl font-black">{propertyType || "Property"}</h3>
                  <div className="mt-5 grid gap-3">
                    <MiniLine label="Location" value={location.name} />
                    <MiniLine label="Tenure" value={tenure || "--"} />
                    <MiniLine label="Area" value={`${area} sqm`} />
                    <MiniLine label="Rooms" value={`${bedrooms} beds • ${bathrooms} baths • ${livingRooms} living`} />
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="mt-8 rounded-[2rem] border border-white/10 bg-white/[0.06] p-7 shadow-2xl backdrop-blur-2xl">
          <div className="mb-7 flex items-center justify-between">
            <div>
              <p className="text-sm text-slate-400">Nearest Comparable Properties</p>
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              {result.comparables.slice(0, 8).map((item, index) => (
                <motion.div
                  ...
                  onMouseEnter={() =>
                    setActiveInsight(
                      `Comparable ${index + 1}: ${formatCurrency(item.saleEstimate_currentPrice)}, ${item.floorAreaSqM} sqm, ${item.bedrooms} beds.`
                    )
                  }
                  className="rounded-3xl border border-white/10 bg-black/30 p-5 transition hover:bg-white/[0.08]"
                >
                  <p className="truncate text-sm text-slate-400">{item.propertyType || "Comparable"}</p>
                  <h3 className="mt-2 text-2xl font-black text-cyan-300">{formatCurrency(item.saleEstimate_currentPrice)}</h3>
                  <p className="mt-3 text-sm text-slate-300">
                    {item.floorAreaSqM} sqm • {item.bedrooms} beds • {item.bathrooms} baths
                  </p>
                  <p className="mt-2 text-xs text-slate-500">
...
function SummaryCard({ icon, title, value }: { icon: React.ReactNode; title: string; value: string }) {
  return (
    <motion.div whileHover={{ y: -6, scale: 1.02 }} className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-6 shadow-2xl backdrop-blur-2xl">
...
function SmallNumber({ icon, label, value, min, max, onChange }: { icon: React.ReactNode; label: string; value: number; min: number; max: number; onChange: (value: number) => void }) {
  return (
    <div className="rounded-3xl border border-white/10 bg-black/30 p-4">
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => onChange(Math.max(min, value - 1))} className="h-10 w-10 rounded-xl bg-white/10 font-black hover:bg-white/20">-</button>
        <div className="flex-1 text-center text-2xl font-black text-cyan-300">{value}</div>
        <button onClick={() => onChange(Math.min(max, value + 1))} className="h-10 w-10 rounded-xl bg-white/10 font-black hover:bg-white/20">+</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-black/30 p-5">
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="rounded-full bg-cyan-300/10 px-4 py-1 font-black text-cyan-300">{value} {suffix}</span>
      </div>
